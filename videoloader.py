                    ...
                    if self.youtube.is_short_duration(total_time) and is_embeddable:
                        ...
                        try:
                            ...
                            if int(likely_exercise_video) > 0:
                                ...
                                log.log_error("Unlikely exercise video detected", None, video_id, video_title, likely_exercise_video)  # Ensure this function logs the error to the file
                        except Exception as e:
                            self.postgres.conn.rollback()
                            error_traceback = traceback.format_exc()
                            print(f"Rolling back any db updates for video {video_id} - error occurred:", str(e))
                            log.log_error(str(e), error_traceback, video_id, video_title, likely_exercise_video)  # Log any exception that occurs
